...
class DownloadSentinel:
    ...
    def createGPKG(self, data, geoms, outfile, driver_name = "GPKG"):
        ...
        layer.CreateField(ogr.FieldDefn('prod_id', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('prod_identifier', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('prod_quicklook_url', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('prod_download_url', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('origin', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('tileid', ogr.OFTString))    
        layer.CreateField(ogr.FieldDefn('cloudcover', ogr.OFTReal))    
        layer.CreateField(ogr.FieldDefn('datastripid', ogr.OFTString))    
        layer.CreateField(ogr.FieldDefn('orbitnumber', ogr.OFTInteger64))    
        layer.CreateField(ogr.FieldDefn('sourceproduct', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('processingdate', ogr.OFTDateTime))
        layer.CreateField(ogr.FieldDefn('productgroupid', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('operationalmode', ogr.OFTString))    
        layer.CreateField(ogr.FieldDefn('processinglevel', ogr.OFTString))    
        layer.CreateField(ogr.FieldDefn('processorversion', ogr.OFTReal))    
        layer.CreateField(ogr.FieldDefn('granuleidentifier', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('platformshortname', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('instrumentshortname', ogr.OFTString))    
        layer.CreateField(ogr.FieldDefn('relativeorbitnumber', ogr.OFTInteger64))    
        layer.CreateField(ogr.FieldDefn('sourceproductorigindate', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('platformserialidentifier', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('producttype', ogr.OFTString))
        layer.CreateField(ogr.FieldDefn('beginningdatetime', ogr.OFTDateTime))
        layer.CreateField(ogr.FieldDefn('endingdatetime', ogr.OFTDateTime))    
        layer.CreateField(ogr.FieldDefn('area_km', ogr.OFTInteger64))
        layer.CreateField(ogr.FieldDefn('query_geom_wkt', ogr.OFTString))
        ...
        for e,(i,g) in enumerate(zip(data,geoms)):
            ...
            feat.SetField('id', e)
            feat.SetField('prod_id', i[0])
            feat.SetField('prod_identifier', i[1])
            feat.SetField('prod_quicklook_url', i[2])
            feat.SetField('prod_download_url', i[3])
            feat.SetField('origin', i[4])
            feat.SetField('tileid', i[5])
            feat.SetField('cloudcover', i[6])
            feat.SetField('datastripid', i[7])
            feat.SetField('orbitnumber', i[8])
            feat.SetField('sourceproduct', i[9])
            feat.SetField('processingdate', str(i[10]))
            feat.SetField('productgroupid', i[11])
            feat.SetField('operationalmode', i[12])
            feat.SetField('processinglevel', i[13])
            feat.SetField('processorversion', i[14])
            feat.SetField('granuleidentifier', i[15])
            feat.SetField('platformshortname', i[16])
            feat.SetField('instrumentshortname', i[17])
            feat.SetField('relativeorbitnumber', i[18])
            feat.SetField('sourceproductorigindate', i[19])
            feat.SetField('platformserialidentifier', i[20])
            feat.SetField('producttype', i[21])
            feat.SetField('beginningdatetime', str(i[22]))
            feat.SetField('endingdatetime', str(i[23]))
            feat.SetField('area_km', i[24])
            feat.SetField('query_geom_wkt', i[25])
        ...
        if isinstance(total_results, str):
            ...
            if total_results == 0:
                ...
                if res == QMessageBox.Yes:
                    ...
                    out_name = f""" footprints_{sd}_{ed} """.strip()
    ...
    def executeDownloadImages(self):
        ...
        for feat in layer:
            ...
            query_geom_wkt = feat.GetField("query_geom_wkt")
            ...
            if self.dlg.cb_ql.isChecked():
                ...
                if self.dlg.cb_extract_files.isChecked():
                    ...
                    if self.dlg.cb_clip.isChecked():
                        ...
                        if not os.path.isdir(fr"{self.dlg.le_outputFolder_2.text()}\{img_name}\clipped_vrt"):
                            os.mkdir(fr"{self.dlg.le_outputFolder_2.text()}\{img_name}\clipped_vrt")
                        
                        flist_for_vrt_tif = glob(fr"{self.dlg.le_outputFolder_2.text()}\{img_name}\*.tif")
                        flist_for_vrt_jp2 = glob(fr"{self.dlg.le_outputFolder_2.text()}\{img_name}\*.jp2")
        
                        flist_for_vrt = [*flist_for_vrt_tif, *flist_for_vrt_jp2]
        
                        raster = gdal.Open(flist_for_vrt[0])
                        srs_vrt = raster.GetSpatialRef()
                        srs_wkt_vrt = srs_vrt.ExportToWkt()
        
                        raster.FlushCache()
                        raster=None
                        del raster
                        
                        minx, maxx, miny, maxy = self.getTransformedGeometry2(query_geom_wkt, srs_wkt_vrt)
        
                        for img_path in flist_for_vrt:
                            path, name_ = os.path.split(img_path)
                            name = name_.replace("jp2","vrt").replace("tif","vrt")    
                            out_path = os.path.join(path, "clipped_vrt" ,name)
                            
                            vrt_options = gdal.BuildVRTOptions(outputBounds=(minx, miny, maxx, maxy))    
                            my_vrt = gdal.BuildVRT(out_path, [img_path], options=vrt_options)
                            my_vrt.FlushCache()
                            my_vrt = None
                            del my_vrt
